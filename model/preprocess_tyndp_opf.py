...
from maintenance_year import (
    ...
    maintenance_year_week_mapping,
    required_source_weather_years,
    source_weather_slot,
    validate_weather_years,
)
...
DEFAULT_EXEMPT_SINGLE_AC_CONNECTIONS_FROM_MAINTENANCE = True
...
def _build_ac_maintenance_requirements(
    ac_corr: pd.DataFrame,
    *,
    frequency_per_year: int,
    exempt_single_connections: bool,
) -> tuple[dict[str, int], pd.DataFrame]:
    """Assign AC maintenance frequencies using physical circuits per bus pair."""
    base_frequency = max(0, int(frequency_per_year))
    parent_units: dict[str, int] = defaultdict(int)
    rows = list(ac_corr.itertuples(index=False))
    for row in rows:
        parent = str(getattr(row, "parent_corr_id", row.corr_id))
        parent_units[parent] += max(1, int(getattr(row, "n_parallel", 1)))

    frequencies: dict[str, int] = {}
    diagnostics: list[dict[str, Any]] = []
    for row in rows:
        corridor_id = str(row.corr_id)
        parent = str(getattr(row, "parent_corr_id", corridor_id))
        model_units = max(1, int(getattr(row, "n_parallel", 1)))
        units_between_buses = int(parent_units[parent])
        exempt = bool(exempt_single_connections and units_between_buses == 1)
        required_frequency = 0 if exempt else base_frequency
        frequencies[corridor_id] = required_frequency
        diagnostics.append(
            {
                "corridor_id": corridor_id,
                "parent_corridor_id": parent,
                "source_ac_id": str(getattr(row, "source_ac_id", "")),
                "bus_from": str(row.n_from),
                "bus_to": str(row.n_to),
                "physical_units_in_model_element": model_units,
                "physical_units_between_buses": units_between_buses,
                "base_frequency_per_year": base_frequency,
                "required_frequency_per_year": required_frequency,
                "required_maintenance_starts_per_year": required_frequency * model_units,
                "exempt_single_ac_connection": int(exempt),
            }
        )
    ...
    columns = [
        ...
        "bus_from",
        "bus_to",
        "physical_units_in_model_element",
        "physical_units_between_buses",
        "base_frequency_per_year",
        "required_frequency_per_year",
        "required_maintenance_starts_per_year",
        "exempt_single_ac_connection",
    ]
    return frequencies, pd.DataFrame(diagnostics, columns=columns)
...
def _load_inertia_factor_table(path: Path | None) -> tuple[dict[str, float], dict[str, float]]:
    if path is None or not Path(path).exists():
        return {}, {}
    frame = pd.read_csv(path, sep=None, engine="python").rename(columns=str.strip)
    required = {"plant_type_code", "H_s", "loading_factor"}
    missing = required - set(frame.columns)
    if missing:
        raise KeyError(f"{Path(path).name} missing inertia columns: {sorted(missing)}")
    frame = frame.copy()
    frame["plant_type_code"] = frame["plant_type_code"].astype(str).str.strip().str.upper()
    frame["H_s"] = pd.to_numeric(frame["H_s"], errors="coerce")
    frame["loading_factor"] = pd.to_numeric(frame["loading_factor"], errors="coerce")
    frame = frame.dropna(subset=["plant_type_code", "H_s", "loading_factor"])
    inertia_h = {
        str(row.plant_type_code): max(0.0, float(row.H_s))
        for row in frame.itertuples(index=False)
    }
    loading_factor = {
        str(row.plant_type_code): min(1.0, max(0.0, float(row.loading_factor)))
        for row in frame.itertuples(index=False)
    }
    return inertia_h, loading_factor


def _lookup_thermal_inertia_h(
    fuel_code: Any,
    tech_norm: Any,
    inertia_h_by_fuel: Mapping[str, float] | None = None,
) -> float:
    fuel = str(fuel_code or "").strip().upper()
    tech = str(tech_norm or "").strip().upper()
    if inertia_h_by_fuel and fuel in inertia_h_by_fuel:
        return float(inertia_h_by_fuel[fuel])
    if fuel == "B04":
...
def _attach_thermal_inertia_factors(
    thermal_data: dict[str, Any],
    *,
    inertia_h_by_fuel: Mapping[str, float] | None = None,
    inertia_loading_factor_by_fuel: Mapping[str, float] | None = None,
) -> dict[str, Any]:
    inertia_h_by_fuel = {
        str(key).strip().upper(): float(value)
        for key, value in (inertia_h_by_fuel or {}).items()
    }
    inertia_loading_factor_by_fuel = {
        str(key).strip().upper(): min(1.0, max(0.0, float(value)))
        for key, value in (inertia_loading_factor_by_fuel or {}).items()
    }
    ...
    if isinstance(groups_df, pd.DataFrame) and not groups_df.empty and "inertia_h" in groups_df.columns:
        group_inertia_h = {
            str(row.group_id): (
                _lookup_thermal_inertia_h(
                    getattr(row, "fuel_code", ""),
                    getattr(row, "tech_norm", ""),
                    inertia_h_by_fuel,
                )
                if str(getattr(row, "fuel_code", "")).strip().upper() in inertia_h_by_fuel
                else (
                    float(row.inertia_h)
                    if pd.notna(getattr(row, "inertia_h", np.nan))
                    else _lookup_thermal_inertia_h(
                        getattr(row, "fuel_code", ""),
                        getattr(row, "tech_norm", ""),
                        inertia_h_by_fuel,
                    )
    ...
    group_inertia_loading_factor = {
        str(group_id): float(
            inertia_loading_factor_by_fuel.get(
                str(thermal_data.get("group_fuel", {}).get(group_id, "")).strip().upper(),
                1.0,
            )
        )
        for group_id in thermal_data.get("groups", [])
    ...
    if isinstance(units_df, pd.DataFrame) and not units_df.empty and "inertia_h" in units_df.columns:
        plant_inertia_h = {
            str(row.plant_id): (
                float(group_inertia_h.get(str(thermal_data.get("plant_group", {}).get(str(row.plant_id), "")), 0.0))
                if inertia_h_by_fuel
                else (
                    float(row.inertia_h)
                    if pd.notna(getattr(row, "inertia_h", np.nan))
                    else float(
                        group_inertia_h.get(
                            str(thermal_data.get("plant_group", {}).get(str(row.plant_id), "")),
                            0.0,
                        )
                    )
                )
    ...
    if isinstance(groups_df, pd.DataFrame) and not groups_df.empty:
        ...
        groups_df["inertia_h"] = groups_df.apply(
            lambda row: float(
                group_inertia_h.get(
                    str(row["group_id"]),
                    _lookup_thermal_inertia_h(row["fuel_code"], row["tech_norm"], inertia_h_by_fuel),
                )
            ),
            axis=1,
        )
        groups_df["inertia_loading_factor"] = groups_df["group_id"].map(
            lambda group_id: float(group_inertia_loading_factor.get(str(group_id), 1.0))
        )
    ...
    thermal_data["group_inertia_loading_factor"] = group_inertia_loading_factor
...
def _build_sync_area_inertia_data(
    *,
    ...
    for (bus0, bus1) in ac_endpoints.values():
    ...
    for idx, component in enumerate(sorted(components, key=lambda comp: comp[0] if comp else "")):
        ...
        for bus_i in component:
            distances = {bus_i: 0}
            queue: deque[str] = deque([bus_i])
            while queue:
                cur = queue.popleft()
                for nb in sorted(adjacency.get(cur, set()) & component_set):
                    if nb in distances:
                        continue
                    distances[nb] = int(distances[cur]) + 1
                    queue.append(nb)

            for bus_k in component:
                distance = distances.get(bus_k)
                value = 0.0 if distance is None else 1.0 / float(1 + int(distance))
...
def _read_csv(path: Path) -> pd.DataFrame:
    try:
        return pd.read_csv(path, sep=";", low_memory=False).rename(columns=_strip_column_name)
...
def _read_csv_auto(path: Path) -> pd.DataFrame:
    try:
        return pd.read_csv(path, sep=None, engine="python").rename(columns=_strip_column_name)
    except pd.errors.EmptyDataError:
        return pd.DataFrame()
...
def _discover_single_year_input_paths(
    ...
    for spec in SINGLE_YEAR_INPUT_DISCOVERY_SPECS:
        configured_value = (files or {}).get(spec.key)
        if configured_value is not None:
            configured = _resolve_config_input_raw_path(
                base_input_dir,
                files or {},
                spec.key,
                ref_year=ref_year,
...
def _forced_thermal_exclusion_mask(
    ...
    *,
    ...
    country = _column_as_series(df, country_col, "").map(_norm_country)
    fuel = _column_as_series(df, fuel_col, "").map(_norm_fuel_code)
    raw_tech = _column_as_series(df, tech_col, "").astype(str).str.strip().str.upper()
    tech = pd.Series(
        [_norm_revision_tech(value, fuel_code) for value, fuel_code in zip(raw_tech, fuel)],
        index=df.index,
    )
    ...
    for rule in FORCED_THERMAL_EXCLUSION_RULES:
        ...
        mask |= country.eq(rule_country) & (fuel.eq(rule_fuel) | raw_tech.eq(rule_tech) | tech.eq(rule_tech))
    return mask
...
def load_thermal_groups(
    ...
    *,
    ...
    for row in grouped_raw.itertuples(index=False):
        n_old = max(1, round(float(row.n_units)))
...
def _load_source_weather_week_schedule(path: Path) -> pd.DataFrame:
    schedule = _read_csv_auto(path)
    required = {"source_weather_year", "source_week", "peak_timestamp"}
    missing = required - set(schedule.columns)
    if missing:
        raise KeyError(f"{path.name} missing columns: {sorted(missing)}")
    schedule = schedule[["source_weather_year", "source_week", "peak_timestamp"]].copy()
    schedule["source_weather_year"] = pd.to_numeric(
        schedule["source_weather_year"], errors="coerce"
    ).astype("Int64")
    schedule["source_week"] = pd.to_numeric(schedule["source_week"], errors="coerce").astype("Int64")
    schedule["_peak_timestamp_key"] = pd.to_datetime(
        schedule["peak_timestamp"], errors="coerce", utc=True
    )
    schedule = schedule.dropna(
        subset=["source_weather_year", "source_week", "_peak_timestamp_key"]
    ).copy()
    schedule["source_weather_year"] = schedule["source_weather_year"].astype(int)
    schedule["source_week"] = schedule["source_week"].astype(int)
    duplicates = schedule.duplicated(["source_weather_year", "source_week"], keep=False)
    if duplicates.any():
        raise ValueError(
            f"{path.name} contains duplicate source slots: "
            f"{schedule.loc[duplicates, ['source_weather_year', 'source_week']].head(10).to_dict('records')}"
        )
    duplicate_timestamps = schedule.duplicated("_peak_timestamp_key", keep=False)
    if duplicate_timestamps.any():
        raise ValueError(
            f"{path.name} assigns one peak timestamp to multiple source slots: "
            f"{schedule.loc[duplicate_timestamps, ['source_weather_year', 'source_week']].head(10).to_dict('records')}"
        )
    return schedule.drop(columns=["peak_timestamp"])


def _reindex_direct_frame_with_source_schedule(
    frame: pd.DataFrame,
    *,
    timestamp_col: str,
    source_schedule: pd.DataFrame | None,
) -> pd.DataFrame:
    if source_schedule is None:
        return frame
    if timestamp_col not in frame.columns:
        raise KeyError(f"Direct weather data missing timestamp column {timestamp_col!r}.")
    work = frame.copy()
    work["_peak_timestamp_key"] = pd.to_datetime(work[timestamp_col], errors="coerce", utc=True)
    work = work.dropna(subset=["_peak_timestamp_key"]).copy()
    work = work.drop(columns=[column for column in ("weather_year", "week") if column in work.columns])
    work = work.merge(
        source_schedule,
        on="_peak_timestamp_key",
        how="inner",
        validate="many_to_one",
    )
    work = work.rename(
        columns={
            "source_weather_year": "weather_year",
            "source_week": "week",
        }
    )
    return work.drop(columns=["_peak_timestamp_key"])
...
def _load_direct_bus_load_from_membership(
    *,
    ...
    years: list[int],
    num_weeks: int,
    source_week_schedule: pd.DataFrame | None = None,
    ...
    required = {"bus", "allocated_load_mw"}
    required.update({"timestamp"} if source_week_schedule is not None else {"weather_year", "week"})
    missing = required - set(df.columns)
    if missing:
        raise KeyError(f"{load_csv.name} missing columns: {sorted(missing)}")
    df = _reindex_direct_frame_with_source_schedule(
        df,
        timestamp_col="timestamp",
        source_schedule=source_week_schedule,
    )
    ...
    df = df[
        ...
        & df["weather_year"].isin({int(y) for y in years})
        & df["week"].between(1, int(num_weeks))
    ].copy()
    if source_week_schedule is not None:
        expected_slots = {(int(year), week) for year in years for week in range(1, int(num_weeks) + 1)}
        actual_slots = set(zip(df["weather_year"].astype(int), df["week"].astype(int)))
        missing_slots = sorted(expected_slots - actual_slots)
        if missing_slots:
            raise ValueError(f"{load_csv.name} is missing scheduled source slots: {missing_slots[:10]}")
...
def _load_direct_res_availability(
    *,
    ...
    if source_week_schedule is not None:
        ...
        missing_slots = sorted(expected_slots - actual_slots)
        if missing_slots:
            raise ValueError(f"{res_csv.name} is missing scheduled source slots: {missing_slots[:10]}")
    df["week_model"] = df["week"] - 1
...
def _load_weekly_country_bus_availability(
    *,
    ...
    df = df[df["week_model"].isin({int(w) for w in weeks})].copy()
    has_weather_year = "weather_year" in df.columns
    if has_weather_year:
        df["weather_year"] = pd.to_numeric(df["weather_year"], errors="coerce").fillna(-1).astype(int)
        df = df[df["weather_year"].isin({int(y) for y in years})].copy()
    if df.empty:
        return {}, {}, empty_diag
...
def _load_direct_thermal_data(
    *,
    ...
    forced_exclusion = _forced_thermal_exclusion_mask(
        units,
        country_col="plant_country",
        fuel_col="fuel_code",
        tech_col="tech_norm",
    )
    if "country" in units.columns:
        forced_exclusion |= _forced_thermal_exclusion_mask(
            units,
            country_col="country",
            fuel_col="fuel_code",
    ...
    forced_excluded_units = units.loc[forced_exclusion].copy()
    if not forced_excluded_units.empty:
        units = units.drop(index=forced_excluded_units.index).copy()
    ...
    if not forced_excluded_units.empty:
        excluded_detail = (
            forced_excluded_units.groupby(["plant_country", "fuel_code", "tech_norm"], as_index=False)
            .agg(
                target_cap_mw=("installed_capacity_mw", "sum"),
                target_n_units=("plant_id", "count"),
            )
            .rename(columns={"plant_country": "country"})
            .assign(
                candidate_rows=0,
                basis_cap_mw=0.0,
                basis_n_units=0.0,
                fallback_stage="forced_policy_exclusion",
                chp_reference_stage="forced_policy_exclusion",
                chp_assigned_units=0,
                matched=False,
                exclusion_reason="de_lu_nuclear_phaseout_2025",
            )
        )
        detail_df = pd.concat([detail_df, excluded_detail], ignore_index=True, sort=False)
...
def _integer_bus_assignments(
    *,
    ...
    n_units = max(round(float(target_row["n_units"])), 1)
...
def _assign_chp_flags_to_units(
    *,
    ...
    target_chp_units = round(float(n_units) * chp_basis_units / total_basis_units)
...
def _build_bus_shares(
    *,
    ...
    return pd.DataFrame(rows, columns=share_columns), pd.DataFrame(diag_rows, columns=diag_columns)
...
_NESTED_COUNTRY_WEEK_KEYS = (
    "peak_load_week",
    "res_avail",
)

_TUPLE_WEATHER_WEEK_POSITIONS = {
    "peak_load_bus": 2,
    "peak_load_country_bus": 3,
    "res_avail_bus": 2,
    "res_avail_country_bus": 3,
    "bess_cap_bus": 2,
    "bess_cap_country_bus": 3,
    "other_res_cap_bus": 2,
    "other_res_cap_country_bus": 3,
    "other_nonres_cap_bus": 2,
    "other_nonres_cap_country_bus": 3,
    "dsr_cap_bus": 2,
    "dsr_cap_country_bus": 3,
    "hydro_turb_stor": 3,
    "hydro_turb_ror": 2,
    "hydro_turb_stor_bus": 2,
    "hydro_turb_stor_country_bus": 3,
    "hydro_ror_bus": 2,
    "hydro_ror_country_bus": 3,
}


def _remap_nested_country_week_values(
    values: Mapping[int, Mapping[str, Mapping[int, float]]],
    *,
    ...
    remapped: dict[int, dict[str, dict[int, float]]] = {}
    for scenario_year in scenario_years:
        remapped[int(scenario_year)] = {}
        for country in countries:
            remapped[int(scenario_year)][str(country)] = {}
            for model_week in range(int(num_weeks)):
                ...
                try:
                    value = values[int(source_year)][str(country)][int(source_week)]
                except KeyError as exc:
                    raise KeyError(
                        "Missing source weather slot while building maintenance year: "
                        f"scenario_year={scenario_year}, model_week={model_week}, "
                        f"source_year={source_year}, source_week={source_week}, country={country}."
                    ) from exc
                remapped[int(scenario_year)][str(country)][int(model_week)] = float(value)
    return remapped


def _remap_tuple_weather_week_values(
    values: Mapping[tuple[Any, ...], Any],
    *,
    scenario_years: list[int],
    start_week: int,
    num_weeks: int,
    ...
    values_by_slot: dict[tuple[int, int], list[tuple[tuple[Any, ...], Any]]] = defaultdict(list)
    for raw_key, value in values.items():
        key = tuple(raw_key)
        values_by_slot[(int(key[0]), int(key[int(week_position)]))].append((key, value))

    remapped: dict[tuple[Any, ...], Any] = {}
    for scenario_year in scenario_years:
        for model_week in range(int(num_weeks)):
            source_year, source_week = source_weather_slot(
                int(scenario_year),
            ...
            for source_key, value in values_by_slot.get((int(source_year), int(source_week)), []):
                target_key = list(source_key)
                target_key[0] = int(scenario_year)
                target_key[int(week_position)] = int(model_week)
                remapped[tuple(target_key)] = value
    return remapped


def _apply_maintenance_year_profile(
    data: dict[str, Any],
    *,
    profile_key: str,
    scenario_years: list[int],
    ...
    countries = [str(country) for country in data.get("countries", [])]
    for key in _NESTED_COUNTRY_WEEK_KEYS:
        data[key] = _remap_nested_country_week_values(
            data[key],
    ...
    for key, week_position in _TUPLE_WEATHER_WEEK_POSITIONS.items():
        data[key] = _remap_tuple_weather_week_values(
            data.get(key, {}),
            scenario_years=scenario_years,
            start_week=int(start_week),
            num_weeks=int(num_weeks),
            week_position=int(week_position),
        )

    data["years"] = list(scenario_years)
    data["weeks"] = list(range(int(num_weeks)))
    data["maintenance_year_profile"] = str(profile_key)
    data["maintenance_year_start_week"] = int(start_week)
    data["maintenance_year_source_weather_years"] = list(source_years)
    data["maintenance_year_week_mapping"] = maintenance_year_week_mapping(
        scenario_years,
        start_week=int(start_week),
        num_weeks=int(num_weeks),
    )
...
def prepare_year_inputs(
    *,
    ...
    scenario_weather_years = validate_weather_years(
        weather_years,
        profile=maintenance_profile,
        first_weather_year=maintenance_year_first_weather_year,
        last_weather_year=maintenance_year_last_weather_year,
    )
    source_weather_years = required_source_weather_years(
        scenario_weather_years,
    ...
    _opf_log(
        ...
        f"maintenance_year_start_week={maintenance_profile.start_week}"
    )
    requested_countries = _resolve_countries(countries_use)
    scenario_wy_min = int(scenario_weather_years[0])
    scenario_wy_max = int(scenario_weather_years[-1])
    source_wy_min = int(source_weather_years[0])
    source_wy_max = int(source_weather_years[-1])
    ...
    p_weather_week_schedule = _resolve_config_input_path(base_input_dir, files, "WEATHER_WEEK_SCHEDULE")
    ...
    p_inertia_factors = _resolve_config_input_path(base_input_dir, files, "INERTIA_FACTORS")
    ...
    resolved_input_paths: dict[str, Path | None] = {
        ...
        "WEATHER_WEEK_SCHEDULE": p_weather_week_schedule,
        "MAX_REV_PLANTS": p_maxrev,
        "REV_DUR_STD": p_dur_std,
        "REV_DUR_LONG": p_dur_long,
        "HYDRO": p_hydro,
        "INERTIA_FACTORS": p_inertia_factors,
    ...
    if p_weather_week_schedule is not None:
        active_input_keys.add("WEATHER_WEEK_SCHEDULE")
    ...
    source_week_schedule = (
        _load_source_weather_week_schedule(p_weather_week_schedule)
        if p_weather_week_schedule is not None and p_weather_week_schedule.exists()
        else None
    )
    ...
    years = list(source_weather_years)
    ...
    weather_weights = load_year_weights(
        p_weights,
        wy_min=scenario_wy_min,
        wy_max=scenario_wy_max,
    )
    ...
    revision_duration_inputs = {
        ...
        "standard": {
            ...
            "entries_loaded": len(dur_map_std),
        ...
        "long": {
            ...
            "entries_loaded": len(dur_map_long),
    ...
    inertia_h_by_fuel, inertia_loading_factor_by_fuel = _load_inertia_factor_table(p_inertia_factors)
    thermal_data = _attach_thermal_inertia_factors(
        thermal_data,
        inertia_h_by_fuel=inertia_h_by_fuel,
        inertia_loading_factor_by_fuel=inertia_loading_factor_by_fuel,
    )
    _log_step_done(
    ...
    if p_disagg_load is not None and p_disagg_load.exists():
        peak_load, peak_load_country_bus, peak_load_bus, disagg_load_diag, load_diag = _load_direct_bus_load_from_membership(
            ...
            years=years,
            num_weeks=int(num_weeks),
            source_week_schedule=source_week_schedule,
        )
        ...
        load = load_weekly_demand_and_res(
            ...
            num_weeks=num_weeks,
            countries_use=countries,
            wy_min=source_wy_min,
            wy_max=source_wy_max,
        )
    ...
    pd.DataFrame(
        [
            ...
            for key, value in sorted(
                (inertia_h_by_fuel or DEFAULT_THERMAL_INERTIA_H_BY_FUEL).items()
            )
        ]
        + [
            {"category": "loading_factor", "key": key, "value": value}
            for key, value in sorted(inertia_loading_factor_by_fuel.items())
        ]
        + [
        ...
        + [
            {
                "category": "hydro",
                "key": "storage",
                "value": inertia_h_by_fuel.get(
                    "B10", inertia_h_by_fuel.get("B12", DEFAULT_HYDRO_STORAGE_INERTIA_H)
                ),
            },
            {
                "category": "hydro",
                "key": "ror",
                "value": inertia_h_by_fuel.get("B11", DEFAULT_HYDRO_ROR_INERTIA_H),
            },
    ...
    freq_rev_corridor, ac_maintenance_requirements = _build_ac_maintenance_requirements(
        ac_corr,
        frequency_per_year=ac_maint_frequency,
        exempt_single_connections=bool(exempt_single_ac_connections_from_maintenance),
    )
    ac_maintenance_requirements.to_csv(
        output_dir / "opf_ac_maintenance_requirements.csv",
        index=False,
        sep=";",
    )
    exempt_ac_elements = int(ac_maintenance_requirements["exempt_single_ac_connection"].sum())
    exempt_ac_parent_corridors = int(
        ac_maintenance_requirements.loc[
            ac_maintenance_requirements["exempt_single_ac_connection"] == 1,
            "parent_corridor_id",
        ].nunique()
    )
    _opf_log(
        "AC maintenance requirements built: "
        f"model_elements={len(ac_maintenance_requirements)}, "
        f"exempt_single_connections={exempt_ac_parent_corridors}, "
        f"exempt_model_elements={exempt_ac_elements}"
    )
    ...
    data = {
        ...
        "group_inertia_loading_factor": thermal_data["group_inertia_loading_factor"],
        ...
        "hydro_stor_inertia_h": float(
            inertia_h_by_fuel.get("B10", inertia_h_by_fuel.get("B12", DEFAULT_HYDRO_STORAGE_INERTIA_H))
        ),
        "hydro_ror_inertia_h": float(inertia_h_by_fuel.get("B11", DEFAULT_HYDRO_ROR_INERTIA_H)),
        ...
        "ac_maintenance_exempt_single_connection": {
            str(row.corridor_id): bool(row.exempt_single_ac_connection)
            for row in ac_maintenance_requirements.itertuples(index=False)
        },
        "ac_parent_physical_circuits": {
            str(row.parent_corridor_id): int(row.physical_units_between_buses)
            for row in ac_maintenance_requirements.drop_duplicates("parent_corridor_id").itertuples(index=False)
        },
        ...
        "line_maintenance_parameters": {
            ...
            "exempt_single_ac_connections_from_maintenance": bool(
                exempt_single_ac_connections_from_maintenance
            ),
            "ac_exempt_single_connection_parent_corridors": exempt_ac_parent_corridors,
            "ac_exempt_single_connection_model_elements": exempt_ac_elements,
            "dc_frequency_per_year": dc_maint_frequency,
            "dc_duration_weeks": dc_maint_duration,
        },
    }

    data = _apply_maintenance_year_profile(
        data,
        profile_key=maintenance_profile.key,
        scenario_years=scenario_weather_years,
        source_years=source_weather_years,
        start_week=int(maintenance_profile.start_week),
        num_weeks=int(num_weeks),
    )
    pd.DataFrame(data["maintenance_year_week_mapping"]).to_csv(
        output_dir / "opf_maintenance_year_week_mapping.csv",
        index=False,
        sep=";",
    )
    _opf_log(
        "Maintenance-year mapping applied: "
        f"profile={maintenance_profile.key}, start_week={maintenance_profile.start_week}, "
        f"scenario_years={len(scenario_weather_years)}, source_years={source_weather_years}"
    )
